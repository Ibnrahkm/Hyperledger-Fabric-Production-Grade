...
fabric-ca-client enroll -u https://$2:$3@localhost:7054 --caname $1 -M ../crypto/$4/users/$5/msp --csr.hosts '$5,*.propvalchain.com,localhost' --tls.certfiles $FABRIC_CA_SERVER_PATH/tls-cert.pem

fabric-ca-client enroll -u https://$2:$3@localhost:7054 --caname $1 -M ../crypto/$4/users/$5/tls --enrollment.profile tls --csr.hosts '$5,*.propvalchain.com,localhost' --tls.certfiles $FABRIC_CA_SERVER_PATH/tls-cert.pem
